...
export default function ChatWidget() {
  ...
  return (
    // Bottom-right on every breakpoint, stacked above BackToTop with a
    // clear vertical gap (bottom-left overlapped full-width form submit
    // buttons on tall pages like Contact — see git history).
    <div className="fixed bottom-28 right-5 md:right-6 z-50 flex flex-col items-end gap-3 print:hidden">
      {open && (
        <div className="w-72 max-w-[calc(100vw-2.5rem)] rounded-2xl bg-white shadow-2xl ring-1 ring-black/5 overflow-hidden animate-fade-in-up">
          <div className="bg-gradient-to-br from-navy-900 to-navy-700 px-5 py-4 flex items-start gap-3">
            <div className="grid place-items-center w-9 h-9 rounded-full bg-white/10 text-orange shrink-0">
              <Icon name="message-circle" className="!w-[18px] !h-[18px]" />
            </div>
            <div>
      ...
      >
        <Icon name={open ? "close" : "message-circle"} className="!w-6 !h-6" />
      </button>
    </div>
  );
}
